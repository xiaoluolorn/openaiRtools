...
#' @param req httr2 request object
#' @param callback Function to call for each chunk (optional)
#' @return If callback is provided, returns invisible(NULL). Otherwise returns list of parsed SSE data chunks.
#' @keywords internal
handle_stream_response <- function(req, callback = NULL) {
  chunks <- list()
  has_callback <- !is.null(callback)
  ...
  # Return collected chunks only if no callback provided
  if (has_callback) {
    invisible(NULL)
  } else {
    chunks
